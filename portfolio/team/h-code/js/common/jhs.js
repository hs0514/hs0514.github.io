...
$(document).ready(function(){
    $('.select').each(function () {
        $('.select .fairness > p').on('click', function () {
...
new Swiper('.swiper-container', {
    ...
    speed : 1000,
    autoplay : { // 자동 재생
        delay : 3000, // autoplay: 3초 마다 자동 슬라이드
    },
    loop : true,
    effect : 'slide',
    pagination : { 
...
$(window).scroll( function() {
    if ( $(document).scrollTop() > navOffset.top ) {
        ...
        $('.header h1 .night').show().addClass('fixed');
        $('.header .menu li a .nightcart').show().addClass('fixed');
        $('.header .search .nightsearch').show().addClass('fixed');
    }
    else {
        $('.headerwrap').removeClass('fixed');
        $('.header h1 .night').hide().removeClass('fixed');
        $('.header .menu li a .nightcart').hide().removeClass('fixed');
        $('.header .search .nightsearch').hide().removeClass('fixed');
    }
});
// 맨 위로 버튼
$(function(){
